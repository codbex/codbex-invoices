	...
	.controller('PageController', ['$scope', '$http', 'messageHub', 'entityApi', function ($scope, $http, messageHub, entityApi) {
		...
		$scope.openDetails = function (entity) {
			...
			messageHub.showDialogWindow("SalesInvoicePayment-details", {
				...
				optionsAmount: $scope.optionsAmount,
			});
		};
		...
		$http.get("/services/ts/codbex-invoices/gen/api/undefined/undefinedService.ts").then(function (response) {
			$scope.optionsAmount = response.data.map(e => {
				return {
					value: e.Id,
					text: e.Name
				}
		...
		$scope.optionsAmountValue = function (optionKey) {
			for (let i = 0; i < $scope.optionsAmount.length; i++) {
				if ($scope.optionsAmount[i].value === optionKey) {
					return $scope.optionsAmount[i].text;
				}
			}
			return null;
		};
