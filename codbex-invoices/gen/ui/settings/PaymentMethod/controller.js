angular.module('page', ["ideUI", "ideView", "entityApi"])
	.config(["messageHubProvider", function (messageHubProvider) {
		messageHubProvider.eventIdPrefix = 'codbex-invoices.settings.PaymentMethod';
	}])
	.config(["entityApiProvider", function (entityApiProvider) {
		entityApiProvider.baseUrl = "/services/ts/codbex-invoices/gen/api/settings/PaymentMethodService.ts";
	}])
		...
		$scope.loadPage = function (pageNumber) {
			...
			entityApi.count().then(function (response) {
				...
				entityApi.list(offset, limit).then(function (response) {
					if (response.status != 200) {
						messageHub.showAlertError("PaymentMethod", `Unable to list PaymentMethod: '${response.message}'`);
						return;
					}
