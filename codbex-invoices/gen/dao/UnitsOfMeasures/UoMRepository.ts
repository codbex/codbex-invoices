...
export class UoMRepository {
    ...
    private async triggerEvent(data: UoMEntityEvent) {
        const triggerExtensions = await extensions.loadExtensionModules("codbex-invoices-UnitsOfMeasures-UoM", ["trigger"]);
        ...
        producer.topic("codbex-invoices/UnitsOfMeasures/UoM").send(JSON.stringify(data));
    }
}