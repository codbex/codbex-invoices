...
const dao = require("codbex-invoices/gen/dao/invoices/InvoiceItem");
...
rs.service()
	...
	.resource("count/{Invoice}")
		.get(function(ctx, request) {
			let Invoice = parseInt(ctx.pathParameters.Invoice);
			Invoice = isNaN(Invoice) ? ctx.pathParameters.Invoice : Invoice;
			http.sendResponseOk("" + dao.count(Invoice));
		})
		.catch(function(ctx, error) {
            if (error.name === "ForbiddenError") {
                http.sendForbiddenRequest(error.message);
	...
	.resource("{id}")
		.get(function(ctx) {
			...
			if (entity) {
				...
				http.sendResponseNotFound("InvoiceItem not found");
			}
		})
		.produces(["application/json"])
		.catch(function(ctx, error) {
	...
	.resource("")
		.post(function(ctx, request, response) {
			...
			response.setHeader("Content-Location", "/services/js/codbex-invoices/gen/api/InvoiceItem.js/" + entity.Id);
