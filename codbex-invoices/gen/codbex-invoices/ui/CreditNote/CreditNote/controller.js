angular.module('page', ["ideUI", "ideView", "entityApi"])
	...
	.controller('PageController', ['$scope', '$http', 'messageHub', 'entityApi', 'Extensions', function ($scope, $http, messageHub, entityApi, Extensions) {
		...
		Extensions.get('dialogWindow', 'codbex-invoices-custom-action').then(function (response) {
			$scope.pageActions = response.filter(e => e.perspective === "CreditNote" && e.view === "CreditNote" && (e.type === "page" || e.type === undefined));
		});
		...
		$scope.loadPage = function (pageNumber, filter) {
			...
			entityApi.count(filter).then(function (response) {
				if (response.status != 200) {
					messageHub.showAlertError("CreditNote", `Unable to count CreditNote: '${response.message}'`);
					return;
				}
				if (response.data) {
				...
				entityApi.search(filter).then(function (response) {
					if (response.status != 200) {
						messageHub.showAlertError("CreditNote", `Unable to list/filter CreditNote: '${response.message}'`);
						return;
					}
		...
		$scope.deleteEntity = function () {
			...
			messageHub.showDialogAsync(
				'Delete CreditNote?',
				`Are you sure you want to delete CreditNote? This action cannot be undone.`,
				[{
					id: "delete-btn-yes",
					type: "emphasized",
					label: "Yes",
				...
				if (msg.data === "delete-btn-yes") {
					entityApi.delete(id).then(function (response) {
						if (response.status != 204) {
							messageHub.showAlertError("CreditNote", `Unable to delete CreditNote: '${response.message}'`);
							return;
						}
						refreshData();
						$scope.loadPage($scope.dataPage, $scope.filter);
		...
		$http.get("/services/ts/codbex-invoices/gen/codbex-invoices/api/undefined/undefinedService.ts").then(function (response) {
