angular.module('page', ["ideUI", "ideView", "entityApi"])
	...
	.controller('PageController', ['$scope', '$http', 'messageHub', 'entityApi', 'Extensions', function ($scope, $http, messageHub, entityApi, Extensions) {
		...
		Extensions.get('dialogWindow', 'codbex-invoices-custom-action').then(function (response) {
			$scope.pageActions = response.filter(e => e.perspective === "salesinvoice" && e.view === "SalesInvoice" && (e.type === "page" || e.type === undefined));
		});
		...
		$scope.loadPage = function (pageNumber, filter) {
			...
			entityApi.count(filter).then(function (response) {
				...
				entityApi.search(filter).then(function (response) {
					if (response.status != 200) {
						messageHub.showAlertError("SalesInvoice", `Unable to list/filter SalesInvoice: '${response.message}'`);
						return;
					}
		...
		$scope.selectEntity = function (entity) {
			...
			messageHub.postMessage("entitySelected", {
				...
				optionsSalesInvoiceType: $scope.optionsSalesInvoiceType,
		...
		$http.get("/services/ts/codbex-invoices/gen/codbex-invoices/api/entities/SalesInvoiceTypeService.ts").then(function (response) {
		...
		$scope.optionsSalesInvoiceTypeValue = function (optionKey) {
			for (let i = 0; i < $scope.optionsSalesInvoiceType.length; i++) {
				if ($scope.optionsSalesInvoiceType[i].value === optionKey) {
					return $scope.optionsSalesInvoiceType[i].text;
				}
			}
			return null;
		};
