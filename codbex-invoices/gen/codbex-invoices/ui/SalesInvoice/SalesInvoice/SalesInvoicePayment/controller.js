angular.module('page', ['blimpKit', 'platformView', 'platformLocale', 'EntityService'])
	...
	.controller('PageController', ($scope, $http, EntityService, Extensions, LocaleService, ButtonStates) => {
		...
		LocaleService.onInit(() => {
			...
			translated.deleteTitle = LocaleService.t('codbex-invoices:codbex-invoices-model.defaults.deleteTitle', { name: '$t(codbex-invoices:codbex-invoices-model.t.SALESINVOICEPAYMENT)' });
			translated.deleteConfirm = LocaleService.t('codbex-invoices:codbex-invoices-model.messages.deleteConfirm', { name: '$t(codbex-invoices:codbex-invoices-model.t.SALESINVOICEPAYMENT)' });
		});
		//-----------------Custom Actions-------------------//
			...
			EntityService.count(filter).then((resp) => {
				...
				EntityService.search(filter).then((response) => {
					if (optionsSalesInvoiceHasMore) {
						...
						if (optionsSalesInvoiceSearchValues.length > 0) {
							$http.post('/services/ts/codbex-invoices/gen/codbex-invoices/api/SalesInvoice/SalesInvoiceController.ts/search', {
								...
								$scope.optionsSalesInvoice.push(...response.data.map(e => ({
									value: e.Id,
									text: e.Name
								})));
							}, (error) => {
					...
					if (optionsCustomerPaymentHasMore) {
						...
						if (optionsCustomerPaymentSearchValues.length > 0) {
							$http.post('/services/ts/codbex-payments/gen/codbex-payments/api/CustomerPayment/CustomerPaymentController.ts/search', {
								...
								$scope.optionsCustomerPayment.push(...response.data.map(e => ({
									value: e.Id,
									text: e.Name
								})));
							}, (error) => {
					...
					Dialogs.showAlert({
						...
						message: LocaleService.t('codbex-invoices:codbex-invoices-model.messages.error.unableToLF', { name: '$t(codbex-invoices:codbex-invoices-model.t.SALESINVOICEPAYMENT)', message: message }),
						type: AlertTypes.Error
					});
				...
				Dialogs.showAlert({
					title: LocaleService.t('codbex-invoices:codbex-invoices-model.t.SALESINVOICEPAYMENT'),
					message: LocaleService.t('codbex-invoices:codbex-invoices-model.messages.error.unableToCount', { name: '$t(codbex-invoices:codbex-invoices-model.t.SALESINVOICEPAYMENT)', message: message }),
					type: AlertTypes.Error
				});
				console.error('EntityService:', error);
			});
			...
			Dialogs.showDialog({
				...
				if (buttonId === 'delete-btn-yes') {
					EntityService.delete(id).then(() => {
						...
						Dialogs.showAlert({
							title: LocaleService.t('codbex-invoices:codbex-invoices-model.t.SALESINVOICEPAYMENT'),
							message: LocaleService.t('codbex-invoices:codbex-invoices-model.messages.error.unableToDelete', { name: '$t(codbex-invoices:codbex-invoices-model.t.SALESINVOICEPAYMENT)', message: message }),
							type: AlertTypes.Error,
						});
						console.error('EntityService:', error);
					});
		...
		$http.get('/services/ts/codbex-invoices/gen/codbex-invoices/api/SalesInvoice/SalesInvoiceController.ts/count').then((response) => {
			...
			$http.get('/services/ts/codbex-invoices/gen/codbex-invoices/api/SalesInvoice/SalesInvoiceController.ts').then((response) => {
				...
				optionsSalesInvoiceHasMore = optionsSalesInvoiceCount > $scope.optionsSalesInvoice.length;
			}, (error) => {
				console.error(error);
				const message = error.data ? error.data.message : '';
				Dialogs.showAlert({
		...
		$http.get('/services/ts/codbex-payments/gen/codbex-payments/api/CustomerPayment/CustomerPaymentController.ts/count').then((response) => {
			...
			$http.get('/services/ts/codbex-payments/gen/codbex-payments/api/CustomerPayment/CustomerPaymentController.ts').then((response) => {
				...
				optionsCustomerPaymentHasMore = optionsCustomerPaymentCount > $scope.optionsCustomerPayment.length;
			}, (error) => {
				console.error(error);
				const message = error.data ? error.data.message : '';
				Dialogs.showAlert({
