angular.module('page', ['blimpKit', 'platformView', 'EntityService'])
	...
	.controller('PageController', ($scope, EntityService, Extensions, ButtonStates) => {
			...
			EntityService.count(filter).then((resp) => {
				...
				request.then((response) => {
					...
					Dialogs.showAlert({
						...
						message: `Unable to list/filter SalesInvoiceStatus: '${error.message}'`,
				...
				Dialogs.showAlert({
					title: 'SalesInvoiceStatus',
					message: `Unable to count SalesInvoiceStatus: '${error.message}'`,
					type: AlertTypes.Error
				});
				console.error('EntityService:', error);
			});
		...
		$scope.deleteEntity = (entity) => {
			let id = entity.Id;
			Dialog.showDialog({
				title: 'Delete SalesInvoiceStatus?',
				message: `Are you sure you want to delete SalesInvoiceStatus? This action cannot be undone.`,
				buttons: [{
					id: 'delete-btn-yes',
				...
				if (buttonId === 'delete-btn-yes') {
					EntityService.delete(id).then((response) => {
						...
						Dialogs.showAlert({
							title: 'SalesInvoiceStatus',
							message: `Unable to delete SalesInvoiceStatus: '${message}'`,
							type: AlertTypes.Error
						});
						console.error('EntityService:', error);
					});
